...
export class WalrusService {
  ...
  constructor() {
    ...
    this.keypair = Ed25519Keypair.fromSecretKey(
      Uint8Array.from(Buffer.from(config.suiHex, "hex"))
    );
  }
  ...
  async deleteBlobAttributes(blobObjectId: string) {
    try {
      const result = await this.walrusClient.executeDeleteBlobTransaction({
        signer: this.keypair,
        blobObjectId,
      });
      return result;
